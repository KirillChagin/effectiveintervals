function[effints] = calceffectiveintervals_ersp_mean(EEG, eventtypes, timeinterval_left, timeinterval_right, constraint_left, constraint_right, superiority_coef, min_int_len)
...
for i=2:length(intervals)
    ...
    if (contains(1) == 1)
        for j=1:length(intervals{i}(:,1))
            ...
            mean_effint = mean(neg_effints);
            
            %%% Get times back to array indexes
            effint_left = round((times(round(mean_effint(2))) * EEG.srate) / 1000);
