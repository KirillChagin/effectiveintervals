...
function[effints] = calceffectiveintervals_ersp_union(EEG, eventtypes, timeinterval_left, timeinterval_right, constraint_left, constraint_right, superiority_coef, min_int_len)
    ...
    if (contains(1) == 1)
        for j=1:length(intervals{i}(:,1))
            ...
            for f=1:nfreqs
                ...
                %%% Collect effits for all freqs
                neg_effints(f,1) = neg_effint.negentropy;
                neg_effints(f,2) = neg_effint.left;
            ...
            min_left = neg_effints(1,2);
            max_right = neg_effints(1,3);
            
            %%% Get union of effints
            for m=2:rowscnt
                if neg_effints(m,2) < min_left
                    min_left = neg_effints(m,2);
                end
                if neg_effints(m,3) > max_right
                    max_right = neg_effints(m,3);
                end
            end    
            
            %%% Get times back to array indexes
            effint_left = round((times(min_left) * EEG.srate) / 1000);
            effint_right = round((times(max_right) * EEG.srate) / 1000);
            
            %%% ----------------------------------------------------------
            effint = struct('negentropy', neg_effints(1,1), 'left', effint_left, 'right', effint_right, 'offset', offset_param);
            effints{j,i} = effint;
            disp(['i = ',num2str(i),' j = ', num2str(j), ' ersp_int_ms = ', num2str(ersp_timeinterval_ms), ' ersp_int_param = ', num2str(ersp_timeinterval_param), ' right = ', num2str(max_right), ' effint_right = ', num2str(effint_right)])
        end
    else
        for j=1:length(intervals{i}(:,1))
