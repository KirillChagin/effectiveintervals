...
function [outvar1] = eegplot_wrapper(data, varargin); % p1,p2,p3,p4,p5,p6,p7,p8,p9)
...
if ~isstr(data) % If NOT a 'noui' call or a callback from uicontrols
   ...
   try
       ...
       disp('eegplot_wrapper() error: calling convention {''key'', value, ... } error'); return;
   end;	
...
%        % Check  consistency of freqs
  ...
  % ---------------------------------
  defdowncom   = 'eegplot_wrapper(''defdowncom'',   gcbf);'; % push button: create/remove window
  defmotioncom = 'eegplot_wrapper(''defmotioncom'', gcbf);'; % motion button: move windows or display current position
  defupcom     = 'eegplot_wrapper(''defupcom'',     gcbf);';
  defctrldowncom = 'eegplot_wrapper(''topoplot'',   gcbf);'; % CTRL press and motion -> do nothing by default
  ...
  defctrlupcom = ''; % CTRL press and up -> do nothing by default
   ...
   try, g.title; 		    catch, g.title		= ['Scroll activity -- eegplot_wrapper()']; 	end;
   ...
   for index=1:length(gfields)
      ...
      otherwise, error(['eegplot_wrapper: unrecognized option: ''' gfields{index} '''' ]);
      end;
   end;
  ...
  if isstr(g.eloc_file) | isstruct(g.eloc_file)  % Read in electrode names
      ...
      eegplot_wrapper('setelect', g.eloc_file, ax1);
  end;
...
% Five move buttons: << < text > >> 
  ...
  u(5) = uicontrol('Parent',figh, ...
	...
	'Callback', 'eegplot_wrapper(''drawp'',0);' );
...
% Text edit fields: ESpacing
  ...
  u(6) = uicontrol('Parent',figh, ...
	...
	'Callback', 'eegplot_wrapper(''draws'',0);' );
...
% ESpacing buttons: + -
  u(7) = uicontrol('Parent',figh, ...
	...
	'string','+',...
	'FontSize',8,...
	'Callback','eegplot_wrapper(''draws'',1)');
  u(8) = uicontrol('Parent',figh, ...
	...
	'string','-',...
	'FontSize',8,...
	'Callback','eegplot_wrapper(''draws'',2)');
...
cb_envelope = ['g = get(gcbf,''userdata'');'...
    ...
    'eegplot_wrapper(''drawp'',0);clear g;'];
...
u(22) = uicontrol('Parent',figh, ...
  ...
  if ~isempty(g.events)
      u(17) = uicontrol('Parent',figh, ...
                        'Units', 'normalized', ...
                        'Position',posbut(17,:), ...
                        'string', 'Event types', 'callback', 'eegplot_wrapper(''drawlegend'', gcbf)');
  end;
  ...
  uimenu('Parent',m(7),'Label','Edit figure', 'Callback', 'eegplot_wrapper(''noui'');');
  ...
  % Scale Eye %%%%%%%%%
  timestring = ['[OBJ1,FIG1] = gcbo;',...
	        'eegplot_wrapper(''scaleeye'',OBJ1,FIG1);',...
		'clear OBJ1 FIG1;'];
  m(7) = uimenu('Parent',m(1),'Label','Show scale','Callback',timestring);
  
  % Title %%%%%%%%%%%%
  uimenu('Parent',m(1),'Label','Title','Callback','eegplot_wrapper(''title'')')
  ...
  uimenu('Parent',m(2),'Label','Time range to display',...
      'Callback','eegplot_wrapper(''window'')')
  
  % Electrode window %%%%%%%%
  uimenu('Parent',m(2),'Label','Number of channels to display',...
      'Callback','eegplot_wrapper(''winelec'')')
  ...
  uimenu('Parent',m(6),'Label','Load .loc(s) file',...
      'Callback','eegplot_wrapper(''loadelect'');')
  
  % Zooms %%%%%%%%
  zm = uimenu('Parent',m(2),'Label','Zoom off/on');
  commandzoom = [ 'set(gcbf, ''windowbuttondownfcn'', [ ''zoom(gcbf,''''down''''); eegplot_wrapper(''''zoom'''', gcbf, 1);'' ]);' ...
  ...
  uimenu('Parent',zm,'Label','Zoom off', 'separator', 'on', 'callback', ...
     ...
     'clear tmpg;' ]);
  uimenu('Parent',figh,'Label', 'Help', 'callback', 'pophelp(''eegplot_wrapper'');');

  % Events %%%%%%%%
  zm = uimenu('Parent',m(2),'Label','Events');
  complotevent = [ 'tmpg = get(gcbf, ''userdata'');' ...
                  'tmpg.plotevent = ''on'';' ...                  
                  'set(gcbf, ''userdata'', tmpg); clear tmpg; eegplot_wrapper(''drawp'', 0);'];
  comnoevent   = [ 'tmpg = get(gcbf, ''userdata'');' ...
                  'tmpg.plotevent = ''off'';' ...                  
                  'set(gcbf, ''userdata'', tmpg); clear tmpg; eegplot_wrapper(''drawp'', 0);'];
  comeventmaxstring   = [ 'tmpg = get(gcbf, ''userdata'');' ...
                'tmpg.plotevent = ''on'';' ...
                'set(gcbf, ''userdata'', tmpg); clear tmpg; eegplot_wrapper(''emaxstring'');']; % JavierLC      
  comeventleg  = [ 'eegplot_wrapper(''drawlegend'', gcbf);'];
...
%  set(figh, 'busyaction', 'cancel');
  ...
  eegplot_wrapper('scaleeye', [], gcf);
  if strcmp(lower(g.scale), 'off')
	  eegplot_wrapper('scaleeye', 'off', gcf);
  end;
  
  eegplot_wrapper('drawp', 0);
  eegplot_wrapper('drawp', 0);
  if g.dispchans ~= g.chans
  	   eegplot_wrapper('zoom', gcf);
  end;  
  eegplot_wrapper('scaleeye', [], gcf);
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  case 'drawp' % Redraw EEG and change position
    ...
    % this test help to couple eegplot_wrapper windows
    if exist('p3', 'var')
    	figh = p3;
    	figure(p3);
    else	
    ...
    % channels in the eegplot_wrapper figure window.
    for i = 1:g.chans
...
%        keyboard;  
    ...
    set(ax1, 'ylim',[g.elecoffset*g.spacing (g.elecoffset+g.dispchans+1)*g.spacing] );
	 ...
	 if g.children ~= 0
		...
		eegplot_wrapper( 'drawp', p1, p2, g.children);
		figure(figh);
	 end;	  
...
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % 
% % % % % % % % THIS CODE WAS CHANGED: ADDED CHECK FOR 0  % % % % % % % % % % % % % % % %
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %                     
                    if (poscumul ~= 0)
...
{ 'Number of channels to display:' } , 'Change number of channels to display', 1,  { num2str(g.dispchans) });
   ...
   set(gcf, 'UserData', g);
   eegplot_wrapper('updateslider', fig);
   eegplot_wrapper('drawp',0);	
   eegplot_wrapper('scaleeye', [], fig);
   return;
  ...
  case 'loadelect' % load channels
	...
	eegplot_wrapper('setelect',[ inputpath inputname ],AXH0);
	return;
  ...
  case 'noui'
      if ~isempty(varargin)
          eegplot_wrapper( varargin{:} ); fig = gcf;
      else 
   ...
   case 'zoom' % if zoom
      ...
      % ------------------------------
      if exist('p2', 'var') == 1
          set(gcbf, 'windowbuttondownfcn', [ 'zoom(gcbf,''down''); eegplot_wrapper(''zoom'', gcbf, 1);' ]);
    ...
    if strcmp(get(fig, 'SelectionType'),'normal');
        ...
        if g.incallback ~= 1 % interception of nestest calls
            ...
            if (tmppos(1) >= 0) & (tmppos(1) <= highlim),
                ...
                set(fig,'UserData', g);
                eegplot_wrapper('drawp', 0);  % redraw background
            end;
        end;
    end;      
   otherwise
      error(['Error - invalid eegplot_wrapper() parameter: ',data])
  end  
end
